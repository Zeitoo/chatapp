...
export async function handleWebSocketConnection(ws: AuthenticatedSocket, req: any) {
...
async function handleNewMessage(data: WebSocketMessage, senderUserId: string, senderWs: WebSocket) {
  ...
  const response = await putMessage({ chatId, userId: senderUserId, conteudo });
  ...
  for (const target of targets) {
    try {
      ...
      console.error(`Erro ao enviar mensagem para usuário ${target.userId}:`, error);
    }
  }
