...
export function validateAndSanitizeName(
	...
	if (sanitized.length < 2) {
		return {
			name: sanitized,
			isValid: false,
			error: `${field} No min. 2 caracteres`,
		};
	}
	...
	if (sanitized.length > 30) {
		return {
			name: sanitized,
			isValid: false,
			error: `${field} Muito longo (máximo 30 caracteres)`,
		};
	}
...
export function validateAndSanitizePassword(password: string): {
	...
	if (sanitized.length < 8) {
		return {
			password: sanitized,
			isValid: false,
			error: "Deve ter no Min. 8 caracteres",
		};
	}
	...
	if (!hasUpperCase || !hasLowerCase || !hasNumbers || !hasSpecialChar) {
		return {
			password: sanitized,
			isValid: false,
			error: "Deve conter letras maiúsculas, números e caracteres especiais",
		};
	}
